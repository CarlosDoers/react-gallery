...
uniform vec2 u_mouse;    // Mouse position in UV space
out vec4 fragColor;
...
float mapScene(vec2 uv) {
  ...
  float bMouse = sdCircle(uv - u_mouse, 0.15); // Mouse blob

  float u12 = opSmoothUnion(b1, b2, u_smoothK.x);
  float u123 = opSmoothUnion(u12, b3, u_smoothK.y);
  return opSmoothUnion(u123, bMouse, u_smoothK.x); // Merge mouse blob
}
...
const LiquidCrystalBackground = ({
  ...
  useEffect(() => {
    ...
    class Renderer {
      ...
      render(timeMs, mousePos) {
        ...
        gl.uniform2fv(this.uMouse, mousePos);
    ...
    const handleMouseMove = (e) => {
      ...
      // Convert to UV space centered at 0,0, scaled by height
      // Matches shader: (gl_FragCoord.xy - 0.5 * u_resolution.xy) / u_resolution.y
      const w = canvas.width / (window.devicePixelRatio || 1);
      const h = canvas.height / (window.devicePixelRatio || 1);
      
      mouseX = (x - w * 0.5) / h;
      mouseY = -(y - h * 0.5) / h; // Flip Y
    };
    window.addEventListener("mousemove", handleMouseMove);

    let rafId;
    const animate = (t) => {
      renderer.render(t, [mouseX, mouseY]);
  ...
  return (
    <div
      ...
      className={`liquid-crystal-container ${className}`}
    >
...
LiquidCrystalBackground.propTypes = {
  speed: PropTypes.number,
  radii: PropTypes.arrayOf(PropTypes.number),
  smoothK: PropTypes.arrayOf(PropTypes.number),
