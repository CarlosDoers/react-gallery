...
export default function Waves({
    ...
    const setLines = () => {
        ...
        const xGap = 10;  // Increased from 8 for performance
        const yGap = 10;  // Increased from 8 for performance
        ...
        lines.forEach((points) => {
            points.forEach((p) => {
                ...
                if (d < l) {
                    ...
                    p.cursor.vx += Math.cos(mouse.a) * f * l * mouse.vs * 0.001;  // Increased influence
                    p.cursor.vy += Math.sin(mouse.a) * f * l * mouse.vs * 0.001;  // Increased influence
                }

                p.cursor.vx += (0 - p.cursor.x) * 0.005;   // Reduced restoration force (more fluid)
                p.cursor.vy += (0 - p.cursor.y) * 0.005;   // Reduced restoration force (more fluid)

                p.cursor.vx *= 0.96;  // Increased glide (less friction)
                p.cursor.vy *= 0.96;  // Increased glide (less friction)
    ...
    // Update stroke color when prop changes
    useEffect(() => {
        const { current: paths } = pathsRef;
        paths.forEach(path => {
            path.setAttribute('stroke', strokeColor);
